...
"""WBD 10-digit watersheds for Stage C spatial blocks and the train halo."""
...
from calumetmap.align import require_live_template, write_aligned
...
from calumetmap.fetch import GetJson, default_get_json
from calumetmap.template import TemplateGrid


def huc10_query_url(*, huc8: str = HUC8) -> str:
    where = "huc10 LIKE '{0}%'".format(huc8)
    return (
        "{0}/query?where={1}&outFields=huc10,name,areasqkm,states"
...
def fetch_huc10_features(
    ...
    *,
    ...
    if doc.get("error"):
        raise FetchError("HUC-10 query error: {0}".format(doc["error"]))
    feats = doc.get("features") or []
    if not feats:
        raise GateError("no HUC-10 polygons for {0}".format(huc8))
    return feats
...
def rasterize_huc10(
    ...
    for feat in features:
        ...
        if not code or not geom_doc:
            continue
        if not code.startswith(HUC8):
            raise GateError("HUC-10 {0} is not inside {1}".format(code, HUC8))
        geom = shape(geom_doc)
        if geom.is_empty:
            continue
        g5070 = shape(
