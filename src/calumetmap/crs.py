...
from calumetmap.config import TEMPLATE_CRS
from calumetmap.errors import CrsMismatchError, CrsMissingError
...
def require_epsg(wkid: int | None, *, expected: int = TEMPLATE_CRS) -> int:
    ...
    if int(wkid) != expected:
        raise CrsMismatchError("CRS EPSG:{0} != EPSG:{1}".format(wkid, expected))
    return int(wkid)
