...
from calumetmap.fetch import fetch_nlcd_tile_bytes, iter_tiles, snap_bounds
...
def huc_geom_5070(huc: HucLayer) -> BaseGeometry:
    if huc.crs != VECTOR_CRS:
        raise GateError("HUC CRS {0} != {1} before warp".format(huc.crs, VECTOR_CRS))
    return shape(
        transform_geom(
...
def write_nlcd_window(
    ...
    *,
    ...
    """Small nlcd_2021 window for CI. Must be larger than the 32x32 fixture."""
    if rows <= FIXTURE_ROWS and cols <= FIXTURE_COLS:
        raise GateError("nlcd window looks like the fixture grid")
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with rasterio.open(path, "w", **profile) as dst:
        dst.write(data, 1)
    return inspect_template(path, kind=kind)
