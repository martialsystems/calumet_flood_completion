...
height to the nearest painted stream pixel. Do not warp Nora HAND here.
"""
...
from calumetmap.config import HYDRO_FILL_EPSILON_M, SLOPE_FLOOR_RAD
...
def require_finite_twi(twi: np.ndarray, valid: np.ndarray) -> None:
    ...
    if not np.isfinite(subset).all():
        ...
        raise GateError("TWI has {0} non-finite interior cells".format(n_bad))
